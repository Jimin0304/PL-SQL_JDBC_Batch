...
/**
 * 배치 처리 성능 비교 - Statement 방식 5단계 (최종 최적화)
 * 특징: Fetch Size 1000으로 변경하여 빈번한 Fetch 연산 개선
 * 개선: Statement 재사용 + SQL 조건절 + 배치 Commit + 최적화된 Fetch Size
 */
public class Calc_Bonus_by_stmt_5 {
    ...
    private static final int FETCH_SIZE = 1000;    // Fetch 단위 (개선!)
    ...
    public static void main(String[] args) {
        ...
        try {
            System.out.println("=== 배치 처리 시작 - Statement 방식 5단계 (최종 최적화) ===");
            System.out.printf("최종 최적화: Fetch Size %,d + Commit Size %,d + Statement 재사용 + SQL 조건절%n",
            ...
            selectStmt.setFetchSize(FETCH_SIZE); // 최적화된 Fetch Size!
            ...
            System.out.printf("Fetch Size: %,d (네트워크 라운드트립 최적화!)%n", FETCH_SIZE);

            rs = selectStmt.executeQuery(selectSQL);
            System.out.println("고객 데이터 조회 시작 (최적화된 Fetch로 빠른 처리 예상)");
            ...
            while (rs.next()) {
                ...
                try {
                    ...
                    // 진행률 출력 (빠른 처리로 더 자주 출력)
                    if (processedCount % 20000 == 0) {
                        System.out.printf("처리 진행률: %,d건 조회됨 (오류: %,d건) - 빠른 Fetch로 가속화!%n",
                    ...
                    if (couponCode != null) {
                        // 7. 최적화된 INSERT 처리
                        if (insertCouponWithoutCommit(insertStmt, customerId, email, couponCode, creditLimit)) {
                            insertCount++;
                            ...
                            if (insertCount % 50000 == 0) {
                                System.out.printf("쿠폰 발급 진행률: %,d건 발급됨 (최적화된 성능!)%n", insertCount);
                            }
                        }
                    }
    ...
    /**
     * Commit 없이 INSERT만 수행 (배치 단위로 나중에 Commit)
     */
    private static boolean insertCouponWithoutCommit(Statement insertStmt, String customerId, String email,
                                                     String couponCode, BigDecimal creditLimit) {
        try {
            // 동일한 Statement 객체 재사용, Commit은 하지 않음
    ...
    private static void printResults(long startTime, int processedCount, int insertCount,
        ...
        // 최종 성능 최적화 분석
        System.out.println("\n=== 최종 성능 최적화 분석 ===");
        System.out.printf("• 데이터 전송량: %,d건 (SQL 조건절로 최적화)%n", processedCount);
        System.out.printf("• Statement 객체 생성: 1회 (재사용으로 최적화)%n");
        System.out.printf("• Commit 횟수: %d회 (배치 처리로 최적화)%n", commitCount);
        System.out.printf("• Fetch Size: %,d건 (네트워크 최적화)%n", FETCH_SIZE);

        // 네트워크 최적화 효과
        int actualNetworkCalls = (int) Math.ceil((double)processedCount / FETCH_SIZE);
        int originalNetworkCalls = processedCount / 10; // 기존 Fetch Size 10
        double networkReduction = ((double)(originalNetworkCalls - actualNetworkCalls) / originalNetworkCalls) * 100;
        System.out.printf("• 네트워크 호출 감소: %,d회 → %,d회 (%.1f%% 감소)%n",
                originalNetworkCalls, actualNetworkCalls, networkReduction);
        ...
        System.out.println("\n 최종 달성한 최적화:");
        System.out.println("1. SQL 조건절: 불필요한 데이터 전송 제거");
        System.out.println("2. Statement 재사용: 객체 생성/해제 오버헤드 제거");
        System.out.printf("3. 배치 Commit: 트랜잭션 오버헤드 %d배 감소%n", insertCount/commitCount);
        System.out.printf("4. 최적화된 Fetch: 네트워크 라운드트립 %.0f배 감소%n",
                (double)originalNetworkCalls / actualNetworkCalls);
    }
    ...
    private static void closeResources(ResultSet rs, Statement selectStmt, Statement insertStmt, Connection conn) {
        ...
        if (insertStmt != null) {
            try {
                insertStmt.close();
                System.out.println("INSERT Statement 정리 완료 (최종 최적화 완료)");
            } catch (SQLException e) {
